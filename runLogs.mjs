...
async function getLogs(startDate, endDate, agents, logsNum){
    ...
    try {
        ...
        await io.emit('logUpdateMsg', errMsg);
    }
    ...
    async function getL(){
        for (let index = 0; index < agents.length; index++) {
            const el = agents[index].value;
            let updateMsg = {
                id: uuidv4(),
                title: `Getting Logs For ${el}...`,
                time: new Date(),
                type: 'order3'
            }
            await io.emit('logUpdateMsg', updateMsg);
            ...
            await doc.loadInfo();
            const sheet = doc.sheetsByTitle['SansKia'];
            ...
            try {
                await page.waitForTimeout(1000);
                await page.waitForSelector(`aria/${el}`, {
                    timeout: 5000
                });
                await page.waitForTimeout(1000);
                await page.click(`aria/${el}`);
            ...
            try {
                // select date range
              await page.waitForSelector('#main > div > div.workplace.ng-scope > div > history > div > div.top-panel.ng-scope > div > div.filter-panel.layout-row > div.layout-align-end-center.layout-row.flex-60 > md-datepicker:nth-child(1) > div > button > div')
              await page.waitForTimeout(1000);
              await page.click('#main > div > div.workplace.ng-scope > div > history > div > div.top-panel.ng-scope > div > div.filter-panel.layout-row > div.layout-align-end-center.layout-row.flex-60 > md-datepicker:nth-child(1) > div > button > div');
              await page.waitForTimeout(1000);
              // select start date
              await page.waitForSelector(`aria/${startDate1}`)
              await page.waitForTimeout(1000);
              await page.click(`aria/${startDate1}`);
              await page.waitForTimeout(1000);
              // select second date range
              await page.waitForSelector('#main > div > div.workplace.ng-scope > div > history > div > div.top-panel.ng-scope > div > div.filter-panel.layout-row > div.layout-align-end-center.layout-row.flex-60 > md-datepicker:nth-child(3) > div > button');
              await page.waitForTimeout(1000);
              await page.click('#main > div > div.workplace.ng-scope > div > history > div > div.top-panel.ng-scope > div > div.filter-panel.layout-row > div.layout-align-end-center.layout-row.flex-60 > md-datepicker:nth-child(3) > div > button');
              await page.waitForTimeout(1000);
              // select end date
              await page.waitForSelector(`aria/${endDate1}`)
              await page.waitForTimeout(1000);
              await page.click(`aria/${endDate1}`);
              await page.waitForTimeout(1000);
            ...
            try{
                let xn = await page.$x('/html/body/div[1]/div[2]/div/div/div/section/div/div[3]/div/history/div/div[4]/div[1]/md-select/md-select-value/span[2]', {timeout: 10000});
                await page.waitForTimeout(4500);
                await xn[0].click()
                await page.waitForTimeout(2500);
                await page.waitForTimeout(1000);
                await page.waitForSelector('aria/100')
                await page.waitForTimeout(1000);
                await page.click('aria/100');
                await page.waitForTimeout(15000);
            }catch(e){
                try {
                    await page.waitForTimeout(10000);
            ...
            let lastVal = 1;
            let usingLastVal = false;
            let lastVal2 = 1;
            async function getLogsdata(){
                for (let idx = 1; idx < logsNum; idx++) {
                    lastVal2 = idx;
                    let arrVal = idx;
                    if (usingLastVal) {
                        arrVal = idx - lastVal;
                    ...
                    let txt        
                    try {
                    let currentDate = await page.$(`#main > div > div.workplace.ng-scope > div > history > div > div.content-section.ng-scope > conversations > div > div:nth-child(${arrVal}) > div > div.layout-align-start-center.layout-row.flex-15 > span`);
                    txt = await page.evaluate(element => element.textContent, currentDate);
                    } catch (error) {
                        try {
                            let currentDate = await page.$(`#main > div > div.workplace.ng-scope > div > history > div > div.content-section.ng-scope > conversations > div > div:nth-child(${arrVal}) > div > div.layout-align-start-center.layout-row.flex-15 > span`);
                            txt = await page.evaluate(element => element.textContent, currentDate);
                            ...
                            let updateMsg = {
                                id: uuidv4(),
                                type: "order4",
                                title: `Error Getting Logs For ${el}...`,
                                time: new Date()
                            }
                            await io.emit('logUpdateMsg', updateMsg);
                            break;
                    ...
                    // click on conversation head
                    try{
                        await page.waitForSelector(`#main > div > div.workplace.ng-scope > div > history > div > div.content-section.ng-scope > conversations > div > div:nth-child(${arrVal}) > div`, {timeout: 5000})
                        await page.waitForTimeout(1000)
                        ...
                        try {
                            ...
                            // try getting to next page
                            try {
                                await page.waitForTimeout(2000)
                                await page.waitForSelector('aria/navigate_next')
                                await page.waitForTimeout(1000)
                                await page.click('aria/navigate_next');
                                await page.waitForTimeout(15000);
                                lastVal = idx 
                                usingLastVal = true;
                                continue;
                                ...
                                if(arrVal > 1){
                                    let updateMsg = {
                                        id: uuidv4(),
                                        type: "order2",
                                        title: `Done Getting Logs For ${el}...`,
                                        time: new Date()
                                    }
                                    await io.emit('logUpdateMsg', updateMsg);
                                    let updateMsg1 = {
                                        id: uuidv4(),
                                        type: "order1",
                                        title: `${idx} logs logged for ${el}...`,
                                        time: new Date()
                                    } 
                                    await io.emit('logUpdateMsg', updateMsg1);
                                }else{
                                    let updateMsg = {
                                        id: uuidv4(),
                                        type: "order4",
                    ...
                    try {
                        ...
                        let bg = startTime.split(' ')
                        let cg = bg[1].substring(0, bg[1].length - 1)
                        let m = new Date()
                        let mm = m.getMonth() + 1
                        let y = m.getFullYear()
                        let dg = mm + '/' + cg + '/' + y
                    ...
                    try {
                        await page.waitForTimeout(1000);
                        await page.waitForSelector(`#main > div > div.workplace.ng-scope > div > history > div > div.content-section.ng-scope > conversations > div > div:nth-child(${arrVal}) > interactions > div > div.header.layout-align-center-center.layout-row > div.action-buttons.unselectable.flex-45`, {timeout: 15000});
                        await page.waitForTimeout(1000);
                        await page.click(`#main > div > div.workplace.ng-scope > div > history > div > div.content-section.ng-scope > conversations > div > div:nth-child(${arrVal}) > interactions > div > div.header.layout-align-center-center.layout-row > div.action-buttons.unselectable.flex-45`);
                        await page.waitForTimeout(1000);
                    } catch (error) {
                        continue
                    }
            ...
            await io.emit('logUpdateMsg', sendMsgA);
            let sendMsgB = {
                id: uuidv4(),
                type: 'order1',
                title: `${lastVal2} logs logged for ${el}`,
                time: new Date()
            }
            
            await io.emit('logUpdateMsg', sendMsgB);
        }
    }
    await getL();
    await browser.close();
...
app.post('/api/logs/run',async (req, res) => {
    res.status(200).send({
        message: 'Getting logs started',
    })
    const { startDate, endDate, agents, logsNum } = req.body;
    console.log(req.body)
    await getLogs(startDate, endDate, agents, logsNum);
});
